...
public class Golf {
	...
	private static final String PLAYER_2_WIN = "2";
	 ...
	 class Player {
		...
		public void collectWinningsFromPlayer(Player player1) {
			while (!player1.cardsAtStake.isEmpty()) {
				this.cardsToPlay.offer(player1.cardsAtStake.poll());
			}
		}
	 ...
	 static class Card {
		...
		public int getValue() {
		 ...
		 static Card fromString(String valueAndSuit) {
	  ...
	  class GameEngine {
		...
		private void assignWinningsToPlayer(Player player12) {
			player12.collectWinningsFromPlayer(player1);
			player12.collectWinningsFromPlayer(player2);
		}
